...
public class StartInterface {
    ...
    public static void redHotChiliPepper(){
        ...
        MinimumSpanningTree<String,Integer> mst = new MinimumSpanningTree<>(map);
    ...
    public static void TheHand() {
         // Create an instance of MinimumSpanningTree
        MaximumSpanningTree<String,Integer> mst = new MaximumSpanningTree<>(map);
    ...
    public static void StayTheHell(){
        ...
        String source = abc.nextLine();

        System.out.print("Enter the destination location: ");
        String destination = abc.nextLine();

        System.out.print("Enter the identified locations (comma-separated): ");
        String identifiedLocationsInput = abc.nextLine();
        List<String> identifiedLocations = Arrays.asList(identifiedLocationsInput.split(","));

        DijkstraAlgorithm<String, Integer> dijkstra = new DijkstraAlgorithm<>(map);
        List<String> shortestPath = dijkstra.findShortestPath(source, destination, identifiedLocations);
        ...
        System.out.println("(" + calculateTotalDistance(map, shortestPath) + " km)");
    }
    
    private static <T extends Comparable<T>, N extends Comparable<N>> int calculateTotalDistance(Graph<T, N> graph, List<T> path) {
        int totalDistance = 0;
        for (int i = 0; i < path.size() - 1; i++) {
            T source = path.get(i);
            T destination = path.get(i + 1);
            N edgeWeight = graph.getEdgeWeight(source, destination);
            totalDistance += Integer.parseInt(edgeWeight.toString());
        }
